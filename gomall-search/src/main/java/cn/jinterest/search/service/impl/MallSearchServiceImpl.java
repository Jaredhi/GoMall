...
import cn.jinterest.search.feign.ProductFeignService;
import cn.jinterest.search.service.MallSearchService;
import cn.jinterest.search.vo.AttrResponseVo;
import cn.jinterest.search.vo.BrandVo;
...
public class MallSearchServiceImpl implements MallSearchService {
    ...
    public SearchResult search(SearchParam param) {
        ...
        try {
            // 2、执行检索请求
            SearchResponse response = client.search(searchRequest, GomallElasticSearchConfig.COMMON_OPTIONS);
    ...
    private SearchResult buildSearchResult(SearchResponse response, SearchParam param) {
        ...
        if (param.getAttrs() != null && param.getAttrs().size() > 0) {
            List<SearchResult.NavVo> navVos = param.getAttrs().stream().map(item -> {
                ...
                navVo.setLink("http://search.gomall.com/list.html?"+replace);
                return navVo;
            }).collect(Collectors.toList());
